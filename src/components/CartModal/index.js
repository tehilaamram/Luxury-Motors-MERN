import React from 'react';
import './style.css';
import {Link} from "react-router-dom";
import * as localForage from "localforage";
import axios from "axios";
import buyImage from "../../images/label_black_buy.png";
import removeImage from "../../images/trash_recyclebin_empty_closed.png";

class CartModal extends React.Component {
    constructor(props) {
        super(props);
        this.state={
            vehiclesList:[],
        };
         this.renderVehicleList=this.renderVehicleList.bind(this);
         this.addVehicleToForage=this.addVehicleToForage.bind(this);
         this.removeFromCart=this.removeFromCart.bind(this);
         this.updateVehicleStatus=this.updateVehicleStatus.bind(this);
         this.updateVehiclesList=this.updateVehiclesList.bind(this);
    }
    componentDidMount() {
        this.updateVehiclesList();
        if (!this.state.intervalIsSet) {
            let interval = setInterval(this.updateVehiclesList, 6000);
            this.setState({ intervalIsSet: interval });
        }
    }

    updateVehiclesList() {
        localForage.getItem('user').then((user) => {
            if (user.length !== 0) {
            const vehiclesInCart = user.cart;
            fetch(`${process.env.REACT_APP_SERVER_URL}/vehicle/getVehicle`)
                .then((data) => data.json())
                .then((res) => {
                    let temp = []
                    res.data.forEach((item) => {
                        if (vehiclesInCart.includes(item.number))
                            temp.push(item);
                    });
                    this.setState({vehiclesList: temp})
                });
        }
        })
    }

    closeModal(){
        document.getElementById('CartModal').style.display = 'none';
    }
    updateVehicleStatus(id){
        axios.post(`${process.env.REACT_APP_SERVER_URL}/vehicle/updateVehicle`, {
            id:id,
            update: { status: true },
        });
    }
    removeA(arr) {
        var what, a = arguments, L = a.length, ax;
        while (L > 1 && arr.length) {
            what = a[--L];
            while ((ax= arr.indexOf(what)) !== -1) {
                arr.splice(ax, 1);
            }
        }
        return arr;
    }
    removeFromCart(vehicle){
        this.updateVehicleStatus(vehicle._id);
        localForage.getItem('user').then((user) => {
            var cart1=user.cart;
            this.removeA(cart1,vehicle.number);
            axios.post(`${process.env.REACT_APP_SERVER_URL}/user/updateUser`, {
                id: user._id,
                update: {cart: cart1},
            })
                .then((res) => {
                    alert('added');
                    // user.cart.push(vehicle.number);
                    localForage.setItem('user',user);
                });
            this.updateVehiclesList();
        });
    }
    renderVehicleList(){
        return(
            this.state.vehiclesList.map((vehicle,index)=>{
                return(
                    <view key={index} className='catalogItem'>
                        <view className='catalogItemLeftSide'>
                            <div className='CatalogText'>Number: {vehicle.number}</div>
                            <div className='CatalogText'>Area: {vehicle.area}</div>
                            <div className='CatalogText'>Manufacturer: {vehicle.manufacturer}</div>
                            <div className='CatalogText'>Model: {vehicle.model}</div>
                            <div className='CatalogText'>Color: {vehicle.color}</div>
                            <div className='CatalogText'>Engine capacity: {vehicle.engineCapacity}</div>
                            <div className='CatalogText'>Number of seats: {vehicle.seats}</div>
                            <div className='CatalogText'>Engine type: {vehicle.engineType}</div>
                            <div className='CatalogText'>Gearbox: {vehicle.gearbox}</div>
                        </view>
                        <view style={{flexDirection: 'column', display: 'flex'}}>
                            <img src={vehicle.image} className='vehicleCatalogImage' id='outImage' alt=''/>
                            <view style={{flexDirection: 'row', display: 'flex', justifyContent: 'center'}}>
                                {/*<button className='wishListButton' variant="contained" onClick={this.addToCart.bind(this,vehicle)}>Add to cart</button>*/}
                                <div className='wishListButton' style={{margin:'10px'}} onClick={this.addVehicleToForage.bind(this,vehicle)} >
                                    <img style={{width: '29px', height: '30px', backgroundColor:`rgba(255,255,255, 0)`}} alt='' src={buyImage}/>
                                    <Link to={'/add-order'} props={{vehicle: vehicle}} style={{ textDecoration: 'none' }}>
                                    <div style={{paddingTop: '8px'}} variant="contained" >
                                        Buy</div>
                                    </Link>
                                </div>
                                <div className='wishListButton' style={{margin:'10px'}} onClick={this.removeFromCart.bind(this,vehicle)}>
                                    <img style={{width: '29px', height: '30px', backgroundColor:`rgba(255,255,255, 0)`}} alt='' src={removeImage}/>
                                    <div style={{paddingTop: '8px'}} variant="contained" >
                                        Remove from cart</div>
                                </div>
                            </view>


                            {/*<Link to={'/add-order'} props={{vehicle: vehicle}} style={{ textDecoration: 'none' }}>*/}
                            {/*    <button className='actionButton' variant="contained" onClick={this.addVehicleToForage.bind(this,vehicle)}>Order</button>*/}
                            {/*</Link>*/}
                            {/*<button className='actionButton' variant="contained" onClick={this.removeFromCart.bind(this,vehicle)}>Remove from cart</button>*/}
                        </view>
                    </view>
                )
            })
        );
    }
    addVehicleToForage(vehicle)
    {
        localForage.setItem('vehicle',vehicle);
        this.closeModal();
    }
    render() {
        return (
            <div id='CartModal' className="cartModal">
                <div id='cartModalContent' className="cart-modal-content">
                    <div style={{overflow: 'auto'}}>
                    {this.renderVehicleList()}
                    </div>
                    <img style={{width: '29px', height: '30px',alignSelf: 'center', paddingTop: '40px'}} alt='' onClick={this.closeModal} src='data:image/png;base64,'/>
                </div>
            </div>
        );
    }
}

export default CartModal;